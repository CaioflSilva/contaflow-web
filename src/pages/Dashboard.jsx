...
const C = {
  bg: '#080c14', surface: '#0d1117', surface2: '#161b27',
  border: '#1e2736', accent: '#00d4ff', accent2: '#7c3aed',
  green: '#10b981', yellow: '#f59e0b', red: '#ef4444',
  text: '#e2e8f0', muted: '#64748b', card: '#0d1520',
};
    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: C.bg, color: C.text, fontFamily: "'DM Mono', monospace" }}>
      ...
      <aside style={{ width: '220px', minHeight: '100vh', background: C.surface, borderRight: `1px solid ${C.border}`, display: 'flex', flexDirection: 'column', position: 'fixed', top: 0, left: 0, bottom: 0 }}>
        ...
        <nav style={{ flex: 1, padding: '16px 0', overflowY: 'auto' }}>
          {[
            ...
            <div key={section}>
              ...
              {items.map(item => (
                <div key={item.id} onClick={() => setActiveMenu(item.id)} style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '9px 20px', fontSize: '12px', color: activeMenu === item.id ? C.accent : C.muted, cursor: 'pointer', borderLeft: `2px solid ${activeMenu === item.id ? C.accent : 'transparent'}`, background: activeMenu === item.id ? 'rgba(0,212,255,0.06)' : 'transparent', transition: 'all 0.15s' }}>
                  <span style={{ fontSize: '13px' }}>{item.icon}</span>
                  <span style={{ flex: 1 }}>{item.label}</span>
                  {item.badge && typeof item.badge === 'number' && item.badge > 0 && (
                    <span style={{ background: C.red, color: '#fff', borderRadius: '10px', padding: '1px 6px', fontSize: '9px', fontFamily: 'Syne, sans-serif', fontWeight: 700 }}>{item.badge}</span>
                  )}
                  {item.badge === 'PRO' && (
                    <span style={{ background: 'linear-gradient(135deg,#7c3aed,#4f46e5)', color: '#fff', borderRadius: '4px', padding: '1px 5px', fontSize: '8px', fontFamily: 'Syne, sans-serif', fontWeight: 700 }}>PRO</span>
                  )}
                </div>
              ))}
            </div>
        ...
        <div style={{ padding: '16px 20px', borderTop: `1px solid ${C.border}` }}>
          <div style={{ background: 'linear-gradient(135deg,#7c3aed,#4f46e5)', borderRadius: '8px', padding: '12px', marginBottom: '12px' }}>
            <div style={{ fontFamily: 'Syne, sans-serif', fontWeight: 700, fontSize: '12px' }}>Plano Pro</div>
      ...
      <main style={{ marginLeft: '220px', flex: 1, minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ padding: '18px 32px', borderBottom: `1px solid ${C.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: C.surface, position: 'sticky', top: 0, zIndex: 10 }}>
          <div>
            <div style={{ fontFamily: 'Syne, sans-serif', fontWeight: 700, fontSize: '17px', letterSpacing: '-0.3px' }}>Visão Geral Fiscal</div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            <div style={{ width: '32px', height: '32px', borderRadius: '8px', background: 'linear-gradient(135deg,#7c3aed,#00d4ff)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'Syne, sans-serif', fontWeight: 700, fontSize: '12px' }}>C</div>
            <button onClick={() => setShowModal(true)} style={{ padding: '8px 18px', borderRadius: '6px', fontSize: '11px', fontFamily: "'DM Mono', monospace", cursor: 'pointer', border: 'none', background: C.accent, color: '#000', fontWeight: 600, letterSpacing: '0.5px' }}>
              + Novo Cliente
            </button>
          </div>
        </div>
        ...
        <div style={{ padding: '28px 32px', flex: 1 }}>
          ...
          {/* ── CARDS DE DECISÃO (topo absoluto) ── */}
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr) 1.2fr', gap: '16px', marginBottom: '28px' }}>
            ...
            <div style={{ background: 'rgba(239,68,68,0.06)', border: `1px solid rgba(239,68,68,0.3)`, borderRadius: '12px', padding: '20px', position: 'relative', overflow: 'hidden' }}>
              ...
              <div style={{ fontSize: '10px', color: C.red, letterSpacing: '1.5px', textTransform: 'uppercase', marginBottom: '8px', display: 'flex', alignItems: 'center', gap: '6px' }}>
                <RiskDot level="critical" /> Crítico
              </div>
              <div style={{ fontFamily: 'Syne, sans-serif', fontSize: '36px', fontWeight: 800, color: C.red, letterSpacing: '-1px' }}>{obrigacoesAtrasadas.length}</div>
            ...
            <div style={{ background: 'rgba(245,158,11,0.06)', border: `1px solid rgba(245,158,11,0.3)`, borderRadius: '12px', padding: '20px', position: 'relative', overflow: 'hidden' }}>
              ...
              <div style={{ fontSize: '10px', color: C.yellow, letterSpacing: '1.5px', textTransform: 'uppercase', marginBottom: '8px', display: 'flex', alignItems: 'center', gap: '6px' }}>
                <RiskDot level="warning" /> Atenção
              </div>
              <div style={{ fontFamily: 'Syne, sans-serif', fontSize: '36px', fontWeight: 800, color: C.yellow, letterSpacing: '-1px' }}>{obrigacoesVencendo.length}</div>
            ...
            <div style={{ background: 'rgba(16,185,129,0.06)', border: `1px solid rgba(16,185,129,0.3)`, borderRadius: '12px', padding: '20px', position: 'relative', overflow: 'hidden' }}>
              ...
              <div style={{ fontSize: '10px', color: C.green, letterSpacing: '1.5px', textTransform: 'uppercase', marginBottom: '8px', display: 'flex', alignItems: 'center', gap: '6px' }}>
                <RiskDot level="ok" /> Em dia
              </div>
              <div style={{ fontFamily: 'Syne, sans-serif', fontSize: '36px', fontWeight: 800, color: C.green, letterSpacing: '-1px' }}>{clientes.length}</div>
            ...
            <div style={{ background: 'linear-gradient(135deg, rgba(124,58,237,0.12), rgba(79,70,229,0.06))', border: `1px solid rgba(124,58,237,0.4)`, borderRadius: '12px', padding: '20px', position: 'relative', overflow: 'hidden' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '8px' }}>
                ...
                <span style={{ background: 'linear-gradient(135deg,#7c3aed,#4f46e5)', color: '#fff', borderRadius: '4px', padding: '2px 6px', fontSize: '8px', fontFamily: 'Syne, sans-serif', fontWeight: 700 }}>PRO</span>
              </div>
              <div style={{ fontFamily: 'Syne, sans-serif', fontSize: '28px', fontWeight: 800, color: '#fff', letterSpacing: '-1px' }}>R$ 2.300</div>
              ...
              <button style={{ marginTop: '12px', padding: '6px 12px', borderRadius: '6px', fontSize: '10px', fontFamily: "'DM Mono', monospace", cursor: 'pointer', border: `1px solid rgba(124,58,237,0.5)`, background: 'rgba(124,58,237,0.15)', color: C.accent2, letterSpacing: '0.5px' }}>
                Ver análise →
              </button>
            </div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: '20px', marginBottom: '24px' }}>
            ...
            <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: '12px', overflow: 'hidden' }}>
              <div style={{ padding: '16px 20px', borderBottom: `1px solid ${C.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div>
                  <div style={{ fontFamily: 'Syne, sans-serif', fontWeight: 700, fontSize: '13px' }}>Obrigações — Próximos 30 dias</div>
                  <div style={{ fontSize: '10px', color: C.muted, marginTop: '2px' }}>Ordenadas por urgência e risco</div>
                </div>
                <span style={{ fontSize: '10px', color: C.accent, cursor: 'pointer' }}>Ver todas →</span>
              </div>
              ...
              {[...obrigacoesAtrasadas, ...obrigacoesVencendo].length === 0 ? (
                <div style={{ padding: '48px', textAlign: 'center', color: C.green }}>
                  ...
                  <div style={{ fontSize: '10px', color: C.muted, marginTop: '4px' }}>Todos os clientes estão em dia</div>
                </div>
              ) : (
                [...obrigacoesAtrasadas, ...obrigacoesVencendo].map((ob) => {
                  ...
                  return (
                    <div key={ob.id} style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr', padding: '14px 20px', borderBottom: `1px solid ${C.border}`, alignItems: 'center', fontSize: '11px', cursor: 'pointer', transition: 'background 0.1s' }}>
                      <div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                          <RiskDot level={risk} />
                          <span style={{ fontWeight: 500, color: C.text }}>{cliente?.nome || 'Cliente'}</span>
                        </div>
                        <div style={{ fontSize: '10px', color: C.muted, marginTop: '2px', paddingLeft: '14px' }}>{cliente?.cpfCnpj}</div>
                      </div>
                      <div>
                        <span style={{ display: 'inline-flex', padding: '3px 8px', borderRadius: '4px', fontSize: '10px', fontWeight: 500, background: 'rgba(0,212,255,0.08)', color: C.accent, border: '1px solid rgba(0,212,255,0.2)' }}>
                          {ob.tipo}
                        </span>
                      </div>
            ...
            <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: '12px', overflow: 'hidden' }}>
              <div style={{ padding: '16px 20px', borderBottom: `1px solid ${C.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div style={{ fontFamily: 'Syne, sans-serif', fontWeight: 700, fontSize: '13px' }}>🔔 Alertas Ativos</div>
                <span style={{ fontSize: '10px', color: C.accent, cursor: 'pointer' }}>Configurar</span>
              </div>
              <div style={{ maxHeight: '360px', overflowY: 'auto' }}>
                {obrigacoesAtrasadas.map((ob) => {
                  const cliente = clientes.find(c => c.id === ob.clienteId);
                  return (
                    <div key={ob.id} style={{ padding: '14px 20px', borderBottom: `1px solid ${C.border}`, display: 'flex', gap: '12px', cursor: 'pointer' }}>
                      <div style={{ width: '8px', height: '8px', borderRadius: '50%', background: C.red, boxShadow: `0 0 8px ${C.red}`, marginTop: '5px', flexShrink: 0 }} />
                      <div>
                        <div style={{ fontSize: '11px', lineHeight: 1.6 }}><strong>{ob.tipo}</strong> de <strong>{cliente?.nome}</strong> está atrasado</div>
                ...
                {obrigacoesVencendo.map((ob) => {
                  ...
                  return (
                    <div key={ob.id} style={{ padding: '14px 20px', borderBottom: `1px solid ${C.border}`, display: 'flex', gap: '12px', cursor: 'pointer' }}>
                      <div style={{ width: '8px', height: '8px', borderRadius: '50%', background: isUrgent ? C.red : C.yellow, boxShadow: `0 0 8px ${isUrgent ? C.red : C.yellow}`, marginTop: '5px', flexShrink: 0 }} />
                      <div>
                        <div style={{ fontSize: '11px', lineHeight: 1.6 }}><strong>{ob.tipo}</strong> de <strong>{cliente?.nome}</strong> vence em {dias}d</div>
                ...
                <div style={{ padding: '14px 20px', display: 'flex', gap: '12px' }}>
                  <div style={{ width: '8px', height: '8px', borderRadius: '50%', background: C.accent, boxShadow: `0 0 8px ${C.accent}`, marginTop: '5px', flexShrink: 0 }} />
                  <div>
                    <div style={{ fontSize: '11px', lineHeight: 1.6 }}>🤖 <strong>IA</strong> identificou economia de R$ 2.300 em otimização tributária</div>
                    <div style={{ fontSize: '10px', color: C.accent, marginTop: '3px', cursor: 'pointer' }}>Ver análise completa →</div>
                  </div>
                </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
            ...
            <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: '12px', overflow: 'hidden' }}>
              <div style={{ padding: '16px 20px', borderBottom: `1px solid ${C.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div>
                  <div style={{ fontFamily: 'Syne, sans-serif', fontWeight: 700, fontSize: '13px' }}>Clientes</div>
                  <div style={{ fontSize: '10px', color: C.muted, marginTop: '2px' }}>{clientesRisco.length} com pendências</div>
                </div>
                <span style={{ fontSize: '10px', color: C.accent, cursor: 'pointer' }}>Ver todos →</span>
              </div>
              {clientes.length === 0 ? (
                ...
                clientes.map((cliente, i) => {
                  ...
                  return (
                    <div key={cliente.id} style={{ padding: '14px 20px', borderBottom: `1px solid ${C.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                        <div style={{ width: '32px', height: '32px', borderRadius: '8px', background: avatarGradients[i % avatarGradients.length], display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'Syne, sans-serif', fontWeight: 700, fontSize: '11px', flexShrink: 0 }}>
                          {getClienteInitials(cliente.nome)}
                        </div>
                        <div>
                          <div style={{ fontSize: '12px', fontWeight: 500 }}>{cliente.nome}</div>
            ...
            <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: '12px', overflow: 'hidden' }}>
              <div style={{ padding: '16px 20px', borderBottom: `1px solid ${C.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div>
                  <div style={{ fontFamily: 'Syne, sans-serif', fontWeight: 700, fontSize: '13px' }}>Obrigações por Mês</div>
                ...
                <div style={{ display: 'flex', alignItems: 'flex-end', gap: '8px', height: '100px', marginBottom: '12px' }}>
                  {[
                    ...
                    <div key={mes} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '6px' }}>
                      {pico && <div style={{ fontSize: '8px', color: C.muted, letterSpacing: '0.5px' }}>Pico</div>}
      ...
      {/* ── MODAL ── */}
      {showModal && (
        <div onClick={() => setShowModal(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.75)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', backdropFilter: 'blur(6px)' }}>
          <div onClick={e => e.stopPropagation()} style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: '16px', width: '480px', maxWidth: '95vw', overflow: 'hidden' }}>
            <div style={{ padding: '22px 26px', borderBottom: `1px solid ${C.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div>
                <div style={{ fontFamily: 'Syne, sans-serif', fontWeight: 700, fontSize: '16px' }}>Adicionar Novo Cliente</div>
                <div style={{ fontSize: '10px', color: C.muted, marginTop: '2px' }}>Preencha os dados do escritório ou empresa</div>
              </div>
              <button onClick={() => setShowModal(false)} style={{ background: 'none', border: 'none', color: C.muted, fontSize: '20px', cursor: 'pointer', padding: '4px' }}>✕</button>
            </div>
            <div style={{ padding: '24px 26px' }}>
              {[['CNPJ / CPF', 'text', '00.000.000/0001-00'], ['Razão Social', 'text', 'Nome da empresa'], ['E-mail do Cliente', 'email', 'contato@empresa.com.br'], ['WhatsApp', 'text', '(81) 99999-9999']].map(([label, type, ph]) => (
                <div key={label} style={{ marginBottom: '16px' }}>
                  <label style={{ fontSize: '10px', letterSpacing: '1px', textTransform: 'uppercase', color: C.muted, marginBottom: '6px', display: 'block' }}>{label}</label>
                  <input type={type} placeholder={ph} style={{ width: '100%', background: C.bg, border: `1px solid ${C.border}`, borderRadius: '6px', padding: '10px 14px', fontSize: '12px', fontFamily: "'DM Mono', monospace", color: C.text, outline: 'none', boxSizing: 'border-box' }} />
                </div>
              ))}
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                {[['Regime Tributário', ['Simples Nacional', 'Lucro Presumido', 'Lucro Real', 'MEI']], ['Responsável', ['Ana Santos', 'Carlos Melo', 'João Pereira']]].map(([label, opts]) => (
                  <div key={label}>
                    <label style={{ fontSize: '10px', letterSpacing: '1px', textTransform: 'uppercase', color: C.muted, marginBottom: '6px', display: 'block' }}>{label}</label>
                    <select style={{ width: '100%', background: C.bg, border: `1px solid ${C.border}`, borderRadius: '6px', padding: '10px 14px', fontSize: '12px', fontFamily: "'DM Mono', monospace", color: C.text, outline: 'none' }}>
            ...
            <div style={{ padding: '16px 26px', borderTop: `1px solid ${C.border}`, display: 'flex', justifyContent: 'flex-end', gap: '10px' }}>
              <button onClick={() => setShowModal(false)} style={{ padding: '9px 18px', borderRadius: '6px', fontSize: '11px', fontFamily: "'DM Mono', monospace", cursor: 'pointer', background: C.surface2, color: C.text, border: `1px solid ${C.border}` }}>Cancelar</button>
              <button style={{ padding: '9px 18px', borderRadius: '6px', fontSize: '11px', fontFamily: "'DM Mono', monospace", cursor: 'pointer', border: 'none', background: C.accent, color: '#000', fontWeight: 600 }}>Salvar Cliente</button>
            </div>
          </div>
        </div>
      )}
