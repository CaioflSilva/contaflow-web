...
export default function Configuracoes() {
  ...
  const [perfil, setPerfil] = useState({
    nomeEscritorio: 'Meu Escritório Contábil',
    cnpj: '',
    email: '',
    telefone: '',
    endereco: '',
  ...
  const handleSalvarAlertas = async () => {
    ...
    try {
      ...
      mostrarErro('Erro ao salvar alertas');
    } finally {
      setSalvando(false);
    }
  };
  ...
  const handleAlterarSenha = async () => {
    ...
    try {
      ...
      mostrarErro(e.response?.data?.erro || 'Erro ao alterar senha');
    } finally {
      setSalvando(false);
    }
  };
    ...
    <div style={{ flex: 1, background: C.bg, minHeight: '100vh' }}>
      ...
      <div style={{ padding: '24px 32px', display: 'grid', gridTemplateColumns: '200px 1fr', gap: '24px' }}>
        ...
        <div>
          ...
          {erro && (
            <div style={{ background: '#ef444422', border: `1px solid ${C.red}`, borderRadius: '8px', padding: '12px 16px', marginBottom: '16px', color: C.red, fontSize: '13px', fontWeight: 600 }}>
              ❌ {erro}
            </div>
          )}
            ...
            <div style={{ background: C.surface, borderRadius: '12px', border: `1px solid ${C.border}`, overflow: 'hidden' }}>
              ...
              <div style={{ padding: '24px' }}>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                  ...
                  <div>
                    <label style={labelStyle}>CNPJ do Escritório</label>
                    <input value={perfil.cnpj} onChange={e => setPerfil({ ...perfil, cnpj: e.target.value })} style={inputStyle} placeholder="00.000.000/0001-00" />
                  </div>
                  <div>
                    <label style={labelStyle}>E-mail</label>
                    <input type="email" value={perfil.email} onChange={e => setPerfil({ ...perfil, email: e.target.value })} style={inputStyle} placeholder="contato@escritorio.com.br" />
                  </div>
                  <div>
                    <label style={labelStyle}>Telefone</label>
                    <input value={perfil.telefone} onChange={e => setPerfil({ ...perfil, telefone: e.target.value })} style={inputStyle} placeholder="(81) 99999-9999" />
                  </div>
                  <div>
                    <label style={labelStyle}>Cidade</label>
                    <input value={perfil.cidade} onChange={e => setPerfil({ ...perfil, cidade: e.target.value })} style={inputStyle} placeholder="Recife" />
                  </div>
                  <div>
                    <label style={labelStyle}>Estado</label>
                    <input value={perfil.estado} onChange={e => setPerfil({ ...perfil, estado: e.target.value })} style={inputStyle} placeholder="PE" />
                  </div>
                  <div style={{ gridColumn: '1 / -1' }}>
                    <label style={labelStyle}>Endereço</label>
                    <input value={perfil.endereco} onChange={e => setPerfil({ ...perfil, endereco: e.target.value })} style={inputStyle} placeholder="Rua, número, bairro..." />
                  </div>
                </div>
            ...
            <div style={{ background: C.surface, borderRadius: '12px', border: `1px solid ${C.border}`, overflow: 'hidden' }}>
              ...
              <div style={{ padding: '24px', maxWidth: '400px' }}>
                <div style={{ marginBottom: '16px' }}>
                  ...
                  <input type="password" value={senha.senhaAtual} onChange={e => setSenha({ ...senha, senhaAtual: e.target.value })} style={inputStyle} placeholder="••••••••" />
                </div>
                <div style={{ marginBottom: '16px' }}>
                  <label style={labelStyle}>Nova Senha</label>
                  <input type="password" value={senha.novaSenha} onChange={e => setSenha({ ...senha, novaSenha: e.target.value })} style={inputStyle} placeholder="Mínimo 8 caracteres" />
                </div>
                <div style={{ marginBottom: '24px' }}>
                  <label style={labelStyle}>Confirmar Nova Senha</label>
                  <input type="password" value={senha.confirmarSenha} onChange={e => setSenha({ ...senha, confirmarSenha: e.target.value })} style={inputStyle} placeholder="Repita a nova senha" />
                </div>
                <div style={{ background: C.surface2, borderRadius: '8px', padding: '12px 16px', marginBottom: '20px', border: `1px solid ${C.border}` }}>
                  ...
                  {[
                    ['Mínimo 8 caracteres', senha.novaSenha.length >= 8],
                    ['Letra maiúscula', /[A-Z]/.test(senha.novaSenha)],
                    ['Número', /[0-9]/.test(senha.novaSenha)],
                    ['Caractere especial (@#$!)', /[^a-zA-Z0-9]/.test(senha.novaSenha)],
                  ].map(([req, ok]) => (
                    <div key={req} style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '12px', color: ok ? C.green : C.muted, marginBottom: '3px' }}>
                      <span>{ok ? '✅' : '○'}</span> {req}
                    </div>
                  ))}
                </div>
          ...
          {abaAtiva === 'plano' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              {/* Plano atual */}
              <div style={{ background: 'linear-gradient(135deg, rgba(124,58,237,0.1), rgba(99,102,241,0.05))', borderRadius: '12px', border: `1px solid rgba(124,58,237,0.3)`, padding: '24px' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px' }}>
                  <div>
                    <div style={{ fontFamily: 'Syne, sans-serif', fontWeight: 800, fontSize: '20px', color: C.text }}>Plano Pro</div>
                    <div style={{ fontSize: '13px', color: C.muted, marginTop: '2px' }}>Ativo · Renovação mensal</div>
                  </div>
                  <span style={{ background: 'linear-gradient(135deg,#7c3aed,#4f46e5)', color: '#fff', borderRadius: '8px', padding: '6px 14px', fontSize: '12px', fontWeight: 700 }}>ATIVO</span>
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '12px', marginBottom: '16px' }}>
                  {[
                    { label: 'Clientes', value: '1 / 100' },
                    { label: 'Usuários', value: '1 / 5' },
                    { label: 'Alertas', value: 'Ilimitados' },
                  ].map(item => (
                    <div key={item.label} style={{ background: 'rgba(255,255,255,0.5)', borderRadius: '8px', padding: '12px', textAlign: 'center' }}>
              ...
              <div style={{ background: C.surface, borderRadius: '12px', border: `1px solid ${C.border}`, padding: '24px', textAlign: 'center' }}>
                <div style={{ fontSize: '14px', color: C.muted, marginBottom: '8px' }}>Precisa de mais recursos?</div>
                <div style={{ fontFamily: 'Syne, sans-serif', fontWeight: 700, fontSize: '18px', color: C.text, marginBottom: '16px' }}>Entre em contato conosco</div>
